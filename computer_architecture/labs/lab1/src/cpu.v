...
module cpu(
    ...
    // ??debug_wb_rf_addr???debug_wb_rf_wnum
    ...
    // ??debug_wb_rf_addr???debug_wb_rf_wnum
    assign debug_wb_rf_wnum = write_back_addr;  // ?????????????
    assign debug_wb_rf_wdata = write_back_data;

    // ?????????
    assign inst_sram_en = 1'b1;  // ??????????????
    assign inst_sram_addr = pc_output;  // ??????PC?
    assign imem_output = inst_sram_rdata;  // ????????
    
    // ?????????
    assign data_sram_en = 1'b1;  // ??????????????
    assign data_sram_addr = alureg_output;  // ?????????ALU??
    assign data_sram_wdata = reg_B_output;  // ?????????
    assign dmem_data_output = data_sram_rdata;  // ??????????
    assign data_sram_wen = dmem_write_enable ? 4'b1111 : 4'b0000;  // ??????????
    ...
    IF_ID IF_ID (
        ...
        .ir_input (imem_output),  // ?inst_sram_rdata????
        .pc_input(pc_output),
