目前，我们已经完成了数据库 `flights` 的设计并创建了所有基础表格。下面是对所有表格之间关系和依赖的总结，确保符合最初的实验要求，并检查是否满足范式和关系约束。
...
- **8 个实体**：我们已创建了 8 个实体（表），符合要求。
- **7 个联系**：表格之间的联系（多对多、一对多、一对一）符合要求。
- **完整性约束**：所有关系中都体现了主键约束、外键约束、空值约束，确保了数据的完整性。

下面是你提到的内容的补充部分，添加了一些常用查询的视图、索引、插入与删除操作、连接查询、嵌套查询、分组查询以及 `HAVING` 子句的使用。
...
VALUES ('T111X', 1, 'CR345', '12A', 500.00, 'Economy', '2024-10-24 10:00:00');
```
